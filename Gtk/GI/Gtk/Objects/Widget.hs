...
widgetDragBegin _obj targets actions button event = liftIO $ do
    ...
    result <- gtk_drag_begin _obj' targets' actions' button event'
...
instance (signature ~ (TargetList -> [Gdk.DragAction] -> Int32 -> Gdk.Event -> m Gdk.DragContext), MonadIO m, WidgetK a) => MethodInfo WidgetDragBeginMethodInfo a signature where
...
widgetDragBeginWithCoordinates _obj targets actions button event x y = liftIO $ do
    ...
    let actions' = gflagsToWord actions
    let event' = unsafeManagedPtrGetPtr event
    result <- gtk_drag_begin_with_coordinates _obj' targets' actions' button event' x y
...
instance (signature ~ (TargetList -> [Gdk.DragAction] -> Int32 -> Gdk.Event -> Int32 -> Int32 -> m Gdk.DragContext), MonadIO m, WidgetK a) => MethodInfo WidgetDragBeginWithCoordinatesMethodInfo a signature where
