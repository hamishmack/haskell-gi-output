...
-- function g_io_error_from_win32_error
-- Args : [Arg {argCName = "error_code", argType = TBasicType TInt, direction = DirectionIn, mayBeNull = False, argScope = ScopeTypeInvalid, argClosure = -1, argDestroy = -1, argCallerAllocates = False, transfer = TransferNothing}]
-- Lengths : []
-- returnType : Just (TInterface "Gio" "IOErrorEnum")
-- throws : False
-- Skip return : False

foreign import ccall "g_io_error_from_win32_error" g_io_error_from_win32_error :: 
...
ioErrorFromWin32Error errorCode = liftIO $ do
    result <- g_io_error_from_win32_error errorCode
